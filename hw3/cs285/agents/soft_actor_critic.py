...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            next_qs = torch.roll(next_qs, 1, dims=0)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            obs_flat = obs_expanded.reshape(self.num_actor_samples * batch_size, -1)
            action_flat = action.reshape(self.num_actor_samples * batch_size, self.action_dim)
            q_values_flat = self.critic(obs_flat, action_flat)
            q_values = q_values_flat.view(self.num_critic_networks, self.num_actor_samples, batch_size)
